...
function toOp(op: string | undefined): Operator {
    if (!Object.values(Operator).includes(op as Operator)) {
...
function toFunction(func: string | undefined): MathFunction {
    if (!Object.values(MathFunction).includes(func as MathFunction)) {
...
function toSymbol(symb: string | undefined): MathSymbol | undefined {
    if (!Object.values(MathSymbol).includes(symb as MathSymbol)) {
        return undefined;
    }
...
export function getDependantVariable(expr: MathNode) {
    ...
    if (variables.length < 1) {
        throw new Error("No variable in expression");
    }
    if (variables.length > 1) {
        throw new Error("Too many variables in expression");
    }

    return variables[0];
}

function operatorAsGLSL(args: MathNode[] | undefined, op: Operator): string {
    if (args === undefined) {
...
function symbolAsGLSL(symbol: MathSymbol) {
    switch (symbol) {
        case MathSymbol.i:
            return "i";
...
function constantAsGLSL(val: any | undefined) {
...
function functionAsGLSL(args: MathNode[] | undefined, func: MathFunction): string {
    if (args === undefined) {
